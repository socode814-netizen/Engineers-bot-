...
GENERAL_SEMESTERS_EN = [
    ('الفصل الأول', [
        ('Arabic Language I', 6), ('English Language I', 6), ('General Mathematics I', 12),
        ('Computer Science', 9), ('General Physics I', 9), ('General Statistic', 9),
    ]),
    ('الفصل الثاني', [
...
def admin_main_menu():
    ...
    m.add(
        types.InlineKeyboardButton('🧮 حسابات الطلبة', callback_data='adm_calc_users_0'),
    )
    m.add(
...
def handle_callbacks(call):
    ...
    elif data.startswith('adm_calc_users_'):
        page = int(data.split('_')[-1])
        edit_or_send(call, *build_calc_users_list(page))

    elif data.startswith('calcuser_'):
        target = data.replace('calcuser_', '')
        edit_or_send(call, *build_calc_user_panel(target))
...
# ---------------- حسابات الطلبة (سجل معدلاتي) ----------------
def build_calc_users_list(page):
    history = load_json(HISTORY_FILE, {})
    items = sorted(history.items(), key=lambda x: len(x[1]), reverse=True)
    start, end = page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE
    chunk = items[start:end]

    m = types.InlineKeyboardMarkup()
    if not chunk:
        text = '🧮 لا يوجد طلاب حفظوا معدلاتهم بعد.'
    else:
        text = f'🧮 حسابات الطلبة (صفحة {page + 1}):\nإجمالي الطلاب: {len(items)}'
        for uid, records in chunk:
            m.add(types.InlineKeyboardButton(f'ID {uid} — {len(records)} سجل', callback_data=f'calcuser_{uid}'))

    nav = []
    if page > 0:
        nav.append(types.InlineKeyboardButton('⬅️ السابق', callback_data=f'adm_calc_users_{page-1}'))
    if end < len(items):
        nav.append(types.InlineKeyboardButton('التالي ➡️', callback_data=f'adm_calc_users_{page+1}'))
    if nav:
        m.row(*nav)
    m.add(types.InlineKeyboardButton('⬅️ رجوع للوحة التحكم', callback_data='adm_home'))
    return text, m

def build_calc_user_panel(uid):
    history = load_json(HISTORY_FILE, {})
    records = history.get(uid, [])
    banned = is_banned(uid)
    text = f"🧮 سجل معدلات — ID {uid}\nالحالة: {'🚫 محظور' if banned else '✅ غير محظور'}\n━━━━━━━━━━━━━━━━━━━━\n"
    if records:
        for r in records:
            text += f"• {r['semester']} — {r['type']} — المعدل: {r['gpa']}\n"
    else:
    ...
    m.add(types.InlineKeyboardButton('⬅️ رجوع للقائمة', callback_data='adm_calc_users_0'))
    return text, m
